...
function App() {
  ...
  // all currencies. Could also be done with an API call...
  const currencies = [
  ...
  const toValue = useMemo(() => {
    ...
    return Math.round(fromValue * exchangeRate * 100) / 100;
  }, [fromValue, exchangeRate]);

  return (
    <div className="App">
      <header className={`App-header ${isDark ? "dark" : "light"}`} ref={theme}>
        <Toggle isChecked={isDark} onChange={setIsDark} label="Light/Dark switch"/>
        <CurrencyInput
          label='From'
          value={fromValue}
